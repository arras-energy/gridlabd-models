...
master_dict = {}   # master_dict = {obj_0: val_0, obj_1: val_1, ....... obj_n: val_n}
                               ...
                               #[0 [key,value], 1 [key,value]        ,  ........., n] index location of dictionary
global recorder
...
#        gridlabd.set_global("ica_" + config_globals.iloc[index, 0], str(config_globals.iloc[index, 1]))
    ...
    ica_class_dict = {'underground_line':{'configuration':1,
                                          'rating.summer.continuous':'rating',        # ica_class_dict[0][0] -- 1
                                          'rating.winter.continuous':'rating'},       # ica_class_dict['underground_line']['configuration'] -- 1
    ...
    object_list = gridlabd.get("objects")   # results given after running ieee123.glm, from this we withdraw the objects or results and store them in the variable name
                                            ...
                                            #alert ORDER MUST BE DEFINED OR KNOWN

                                            #object_list = [(0,'a','b'), 1, 2, 3....,n]
    ...
    for obj in object_list:                                                   #object_list = [(0,'a','b'), 1, 2, 3....,n]                                             
        obj_dict.clear()                                                      #                  ^
        ...
        print(obj,obj_class)                                                  # obj_class = classType('meter')
                                                                              #How gridlabd retrieves the 'class' @ documentation
        
        if obj_class in ica_class_dict:                                       # because class is a meter (true) then the if statement is executed 
            class_dict = ica_class_dict.get(obj_class)                        # stores class dictionary associated to ica_class_dict given obj_class equal to meter -- 
            ...
            prop_list = list(class_dict.keys())                               # given 'meter' -- prop_list = ['configuration', nominal_voltage]
            del prop_list[0]                                                        #      at loop                     ^
                                                                                        # investigate del function...
